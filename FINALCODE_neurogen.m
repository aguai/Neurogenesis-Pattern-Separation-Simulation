...
%                           NEUROGENESIS RBM
% -------------------------------------------------------------------------
% Execute this file second.  
%
% NOTE: 
% epsilon (learning rate) = 0.3
% sparsity cost = 0
% weight decay = 0.005
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% length = 10;
% percent = 0.1;
% [orig_data v1 v2 v3 v4 v5 v6 v7 v8 v9 v10] = patternGen(length, percent); 
% 
...
for i = 1:size(visible_data, 1) %i is trial number
    ...
    for k = 1:size(visible_data,2) %1:10
        for m = 1:size(hidden_data,2) %1:50
            % 0.3 replaced epsilon(i)
            delta_weight(k,m) = 0.3*((visible_data(i,k)*hidden_data(i,m))...
                - (visible_recon(i,k)*hidden_recon(i,m))) - ...
                (weightdecay * weight(k,m)) ...
                - (0 * (q(i)-p));% (0 replaced gt_normalized(i));
        end
    end
...
vDiff_PT_neurogen = zeros(1, trialNum);

for i = 1:trialNum
        vDiff_PT_neurogen(i) = 1-(abs(sum(test_visible_data(i,:) - test_visible_recon(i,:)))/trialNum);
end


% VDATA - VRECON / ACCURACY FOR DURING TRAINING ***************************

vDiff_DT_neurogen = zeros(1, trialNum);

for i = 1:trialNum
    vDiff_DT_neurogen(i) = 1-(abs(sum(test_visible_data(i,:) - test_visible_recon(i,:)))/trialNum);
end
...
hiddenOverlapsum_neurogen = zeros(1,trialNum);

for i = 1:trialNum
    hiddenOverlapsum_neurogen(i) = sum(hiddenOverlap(i,:))/trialNum;
end

hiddenOverlapsum_neurogen;
...
axis([0 15 0.5 1.1])
title('Neurogenesis RBM, Post Training Accuracy VS Pattern Number')
...
axis([0 0.1 0 1]);
title('Neurogenesis RBM, Post Training Accuracy VS Overlap')
